...
import org.apache.spark.{ RpcConf, SparkFunSuite}
...
abstract class RpcEnvSuite extends SparkFunSuite with BeforeAndAfterAll {
  ...
  override def beforeAll(): Unit = {
    ...
    val sparkEnv = mock(classOf[RpcEnv])
  ...
  def createRpcEnv(conf: RpcConf, name: String, port: Int, clientMode: Boolean = false): RpcEnv
  ...
  test("send a message remotely") {
    ...
    val anotherEnv = createRpcEnv(new RpcConf(), "remote", 0, clientMode = true)
  ...
  test("ask a message timeout") {
    ...
    try {
      // Any exception thrown in askWithRetry is wrapped with a Exception and set as the cause
      val e = intercept[Exception] {
        rpcEndpointRef.askWithRetry[String]("hello", new RpcTimeout(1 millis, shortProp))
      }
      // The Exception cause should be a RpcTimeoutException with message indicating the
  ...
  test("sendWithReply: error") {
    val endpointRef = env.setupEndpoint("sendWithReply-error", new RpcEndpoint {
      ...
      override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
        case m => context.sendFailure(new Exception("Oops"))
      }
    })
  ...
  test("sendWithReply: remotely error") {
    env.setupEndpoint("sendWithReply-remotely-error", new RpcEndpoint {
      ...
      override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
        case msg: String => context.sendFailure(new Exception("Oops"))
      }
    })
  ...
  test("network events in sever RpcEnv when another RpcEnv is in server mode") {
    val serverEnv1 = createRpcEnv(new RpcConf(), "server1", 0, clientMode = false)
    val serverEnv2 = createRpcEnv(new RpcConf(), "server2", 0, clientMode = false)
  ...
  test("network events in sever RpcEnv when another RpcEnv is in client mode") {
    val serverEnv = createRpcEnv(new RpcConf(), "server", 0, clientMode = false)
    val (serverRef, events) = setupNetworkEndpoint(serverEnv, "network-events")
    val clientEnv = createRpcEnv(new RpcConf(), "client", 0, clientMode = true)
    try {
  ...
  test("port conflict") {
    val anotherEnv = createRpcEnv(new RpcConf(), "remote", env.address.port)
  ...
  test("SPARK-14699: RpcEnv.shutdown should not fire onDisconnected events") {
    ...
    val anotherEnv = createRpcEnv(new RpcConf(), "remote", 0)
