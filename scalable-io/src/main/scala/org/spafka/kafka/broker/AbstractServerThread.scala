...
private[kafka] class Processor(id:Int) extends AbstractServerThread() {
  ...
  private def read(key: SelectionKey) = {
    ...
    logInfo(s"server start read data")
    //1 清空缓冲区旧的数据
    this.readBuf.clear
    //2 获取之前注册的socket通道对象
  ...
  private def configureNewConnections() = {
    ...
    if (!newConnections.isEmpty) {
      ...
      try {
        ...
        logInfo(s"$selectionKey $localHost $localPort $remoteHost $remotePort")
      } catch {
  ...
  override def run() {
    ...
    while (true) {
      ...
      val ready = getSelector.select(500)
